...
for _ in range(Q):
	...
	if q[0] == 1: # 1번 쿼리 (원소 업데이트)
		i = q[1] - 1
		x = q[2]
		update(1, 0, N - 1, i, x) # i번 작물의 크기가 x만큼 늘어난다.
	elif q[0] == 2: # 2번 쿼리 (원소 업데이트)
		i = q[1] - 1
		# i번 작물의 크기가 0이 된다. 즉, i번 작물의 크기는 자기 자신의 크기만큼 줄어드는 것과 같다.
		x = query(1, 0, N - 1, i, i) # i번 작물의 크기를 쿼리로 구한다.
		tot += x # 총 수확량에 i번 작물의 크기를 더한다.
		update(1, 0, N - 1, i, -x) # 구한 크기만큼 감소시킨다.
	else: # 3번 쿼리 (구간 합 쿼리)
		l = q[1] - 1
		r = q[2] - 1
